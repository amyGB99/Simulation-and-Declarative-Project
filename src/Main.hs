...
alterChildren envi (child:xs) gen = let (g1 , actions) =  generateChildActions envi (row child) (column child) gen 
                                            in  let (new_env,g2) = traceShow (("Acciones de los Nenes: ",actions)) ( executeChildActions envi (row child) (column child) actions g1 )
...
executeRobotActions envi row column (action:xs) = case (name action) of  "move" -> let  new =  executeRobotMove envi row column action in executeRobotActions new row column xs
