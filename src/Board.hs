...
printTable env  number_columns number_rows i j string |   i == number_rows && j == number_columns =  env   
                                                          ...
                                                          | (length [ x | x <- robots env, row x == i && column x == j  ] > 0) && (length [ x| x <- dirt env, row x == i && column x == j  ] > 0) = let str = string ++ "|RD " in printTable env number_columns number_rows i (j+1)  str 
